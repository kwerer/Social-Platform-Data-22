...
async function getPage(url, opts) {
    const res = await node_fetch_1.default(new url_1.URL(url), {
        headers: {
...
function getRemoteMetadata(ctx, opts) {
    return async function (metadata) {
        ...
        const target = new url_1.URL(he_1.decode(ctx._oembed.href), ctx.url);
        let res = await node_fetch_1.default(target.href);
        ...
        if (status === 403 && target.protocol === 'http:') {
            ...
            res = await node_fetch_1.default(target.href);
            ...
            const parser = new htmlparser2_1.Parser({
                ...
                onend: function () {
                    if (this._favicon === undefined) {
                        metadata.push(['favicon', new url_1.URL('/favicon.ico', ctx.url).href]);
                    }
                    else {
                        metadata.push(['favicon', new url_1.URL(this._favicon, ctx.url).href]);
                    }
                    resolve(metadata);
                },
    ...
    return function (metadata) {
        ...
        for (let [metaKey, metaValue] of metadata) {
            ...
            if (item.type === 'number') {
                ...
                metaValue = new url_1.URL(metaValue, ctx.url).href;
            }
